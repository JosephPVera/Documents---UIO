...
class Energygradient(LocalEnergy):
    ...
    '''
    ===============================================================================
    Gradient of the wave funtion repect to the parameter alpha
    ===============================================================================
    '''

    # Derivate of wave function ansatz as function of variational parameters
    def derivate_wave_funtion_analytic(self,
        r,
        alpha = None):  
    ...
    # Derivate of wave function ansatz as function of variational parameters
    def derivate_ln_wave_funtion_numeric(self,
        r,
        alpha = None):  

        # Define the gradient with jax ln wave funtion repect a parameter alpha
        Grad_ln_wave_funtion = jax.grad(super().ln_wave_funtion, argnums=1)

        Tota_grad = 0.0

        # Total gradient over all particles
        for i in range(self.Number_particles):
            Evaluated_grad = Grad_ln_wave_funtion(r[i,:],alpha)
    ...
    def energy_derivative_metropolis(self, 
        ...
        # What funtion use analytic or numeric for wave funtion and local energy
        if type(self.Type_calculations) == str:
            if self.Type_calculations == 'analytic':

                # Wave funtion objectType_calculations
                wave_function = super().wave_funtion                  

                # Local energy object
                local_energy = super().local_energy_analytics 

                # Derivative wave funtion object
                derivate_wave_funtion = self.derivate_wave_funtion_analytic
            
            elif self.Type_calculations == 'numeric':

                # Wave funtion object
                wave_function = super().ln_wave_funtion                  

                # Local energy object
                local_energy =super().local_energy_num 

                # Derivative wave funtion object
                derivate_wave_funtion = self.derivate_ln_wave_funtion_numeric
        ...
        for MCcycle in range(Number_MC_cycles):
            ...
            DerPsi = derivate_wave_funtion(Position_old,alpha)
            DeltaPsi += DerPsi
            energy += DeltaE
    ...
    def quantum_force_numeric(self,
        ...
        # Define the gradient with jax ln wave funtion
        Grad_ln_wave_funtion = jax.grad(super().ln_wave_funtion, argnums=0)

        # Calculate the graddient for all particles
        for i in range(self.Number_particles):
            # Gradient
            qforce[i,:] = 2.0*Grad_ln_wave_funtion(r[i,:], alpha) 
